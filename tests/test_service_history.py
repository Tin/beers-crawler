"""Service live-first / history-fallback policy (mocked client)."""
...
async def test_live_success_appends_history(tmp_path):
    ...
    ref = BeerPageRef(query="Pliny", page_url=URL, match_score=1.0, slug="pliny", beer_id="4499")
    meta = BeerMetadata(
        page_url=URL,
    ...
    service = CrawlerService(db, FakeClient(ref=ref, meta=meta))  # type: ignore[arg-type]
...
async def test_live_fail_falls_back_to_history(tmp_path):
    ...
    db.save_page_ref(
        BeerPageRef(query="Pliny", page_url=URL, match_score=0.9, slug="pliny", beer_id="4499")
    )
...
async def test_second_live_crawl_adds_second_history_row(tmp_path):
    ...
    m1 = BeerMetadata(page_url=URL, name="Pliny", rating_score=4.40, scraped_at=datetime(2026, 7, 1, tzinfo=timezone.utc))
    m2 = BeerMetadata(page_url=URL, name="Pliny", rating_score=4.48, scraped_at=datetime(2026, 7, 19, tzinfo=timezone.utc))

    s1 = CrawlerService(db, FakeClient(ref=ref, meta=m1))  # type: ignore[arg-type]
    await s1.crawl_beer("Pliny")
    s2 = CrawlerService(db, FakeClient(ref=ref, meta=m2))  # type: ignore[arg-type]
...
async def test_history_only_skips_live(tmp_path):
    db = BeerDatabase(tmp_path / "h.db")
    db.append_metadata(
        BeerMetadata(page_url=URL, name="Pliny", rating_score=4.1, scraped_at=datetime(2026, 5, 1, tzinfo=timezone.utc))
    )
    ...
    service = CrawlerService(db, client)  # type: ignore[arg-type]
